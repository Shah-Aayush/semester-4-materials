    ...
    def select_add(self, result_id, student_id, exam_id, marks_obtained, result):
        if len(result_id)!=0 and len(student_id)!=0 and len(exam_id)!=0 and len(marks_obtained)!=0 and len(result)!=0:
            ...
            try:
                ...
                mycursor.execute(sql_query)                                     #executing query
                print("s1")
                tv.insert(parent='', index='end', text="Parent", values=(str(result_id), str(student_id), str(exam_id), str(marks_obtained), result))
                print("s2")
                print("inside try : ",mycursor.fetchall())
            except (mysql.connector.Error, mysql.connector.Warning) as e:       #fetching error
                messagebox.showerror("showerror","ERROR MESSAGE :\n" + str(e)) 
...
class ResultModify:
    ...
    def select_modify(self, result_id, student_id, exam_id, marks_obtained, result):
        try:
            ...
            try:
                mycursor.execute(sql_query)  # executing query
                tv.item(tv.focus(), text="", values=(str(result_id), str(student_id), str(exam_id), str(marks_obtained), result))
            except (mysql.connector.Error, mysql.connector.Warning) as e:  # fetching error
                messagebox.showerror("showerror","ERROR MESSAGE\n" + str(e))  # displaying error
