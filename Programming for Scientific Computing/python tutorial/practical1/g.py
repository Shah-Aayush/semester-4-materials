	...
	if(lengthOfString==1):
		...
		if(int(number)<20):
			...
			if(int(number)%10==0):
				...
				return tens[number[0]] + " " + digit[number[1]]
	elif(lengthOfString==3):
		string1 = digit[number[0]] + " Hundred "
		if(int(number)<20):
			return string1 + " "  + one[number[1]]
		else:
			if(int(number)%10==0):
				return string1 + " "  + tens[number[1]]
			else:
				return string1 + " "  + tens[number[1]] + digit[number[2]]
	elif(lengthOfString==4):
		if(int(number)%1000):
			if(int(number)%100):
				return digit[number[0]] + " Thousand "
			return digit[number[0]] + "Thousand"
		
		string1 = digit[number[0]] + " Thousand " + digit[number[1]] + " Hundred "
		if(int(number)<20):
			return string1 + " "  + one[number[2]]
		else:
			if(int(number)%10==0):
				return string1 + tens[number[2]]
			else:
				return string1 + tens[number[2]] + " "  + digit[number[3]]	
...
#	print("Number to words : ",numberToWords(number))
	print("Number to words 2 : ",numberToWords2(str(number)))
